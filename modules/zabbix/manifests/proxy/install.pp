...
#  if $::operatingsystem =~ /(RedHat|CentOS|Fedora|Amazon|SLC)/ {
...
# }
##
  package { "$zabbix::params::proxy_package_name-${zabbix::proxy::db_type}":
 ...
 #       $db_name = $zabbix::proxy::db_name
  #    }
      ...
      $command = "sqlite3 ${db_name} < /usr/share/doc/zabbix-proxy-sqlite3-${zabbix_proxy_ver}/create/schema.sql"
      $creates = $db_name
    }
    'mysql': {
      $command = "mysql -u root ${zabbix::proxy::db_name} < /usr/share/doc/zabbix-proxy-mysql-*/create/schema.sql"
    }
    'postgres': {
