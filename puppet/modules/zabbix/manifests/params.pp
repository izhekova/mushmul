...
class zabbix::params {
  ### commmon parameter
  $zabbix_type               = 'default' #Options: default, mysql, apache, costom
  ...
  $java_gateway              = '127.0.0.1'
  $java_gateway_port         = 10052
  $start_java_pollers        = 10
  $start_vmware_collectors   = 0
  $vmware_frequency          = 60
  $vmware_cache_size         = '8M'
  $smtp_trapper_file         = '/var/log/snmptt/snmptt.log'
  ...
  $agent_alias               = undef
  $agent_bin_dir             = '/etc/zabbix/bin'
  $agent_include             = '/etc/zabbix/zabbix_agentd.d'
  $agent_source_bin_include  = undef #'puppet:///modules/zabbix/bin-default'
  $agent_source_conf_include = undef #'puppet:///modules/zabbix/zabbix_agentd.d-default'
  $unsafe_user_parameters    = 0
  $user_parameter            = undef
  $db_password_uat           = undef
  ...
  case $::operatingsystem {
    ...
    /(RedHat|CentOS|Fedora|Amazon|SLC)/: {
