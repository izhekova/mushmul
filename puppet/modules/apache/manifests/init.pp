...
class apache (
  ...
  $source_jk                 = params_lookup( 'source_jk' ),
  $source_worker             = params_lookup( 'source_worker' ),
  $include_jk                = params_lookup( 'include_jk'),
  ...
  $package_modssl            = params_lookup( 'package_modssl' ),
  ...
  $config_file_jk            = params_lookup( 'config_file_jk' ),
  $config_file_worker        = params_lookup( 'config_file_worker' ),
  ...
  $version_modssl            = params_lookup( 'version_modssl' ),
  $modules_dir               = params_lookup( 'modules_dir'),
  $modules_dir_source        = params_lookup( 'modules_dir_source'), 
  $jk_module                 = params_lookup( 'jk_module'),
  $path_mod_jk               = params_lookup( 'path_mod_jk'),
  $path_ssl_dir              = params_lookup( 'ssl_dir'),
  $source_ssl_dir            = params_lookup( 'source_ssl_dir'),
  $rewrite_rules             = params_lookup( 'rewrite_rules'),
  $rewrite_rules_dir         = params_lookup( 'rewrite_rules_dir'),
  $rewrite_rules_source      = params_lookup( 'rewrite_rules_source'),
#  $ssl_conf_file             = params_lookup( 'ssl_conf_file'),
#  $ssl_conf_file_source      = params_lookup( 'ssl_conf_file_source'),
  ...
  $manage_package_modssl = $apache::bool_absent ? {
    true  => 'absent',
    false => $apache::version_modssl ? {
        ''      => 'present',
        default => $apache::version_modssl,
    },
  }
  ...
  $manage_jk_file_source = $apache::source_jk ? {
    ''        => undef,
    default   => $apache::source_jk,
  }

  $manage_worker_file_source = $apache::source_worker ? {
    ''        => undef,
    default   => $apache::source_worker,
  }

#	$config_file_jk = $apache::config_file_jk ? {
#	  ''			=>	undef,
#	  default	=>	$apache::config_file_jk
#	}
  
#  $config_file_worker = $apache::config_file_worker ? {
#    ''			=>	undef,
#	  default	=>	$apache::config_file_worker
#  }
  
#  $path_ssl_dir = $apache::path_ssl_dir ? {
#		''			=>	undef,
#		default	=>	$apache::path_ssl_dir
#  }
#
#  $source_ssl_dir = $apache::source_ssl_dir ? {
#		''			=>	undef,
#		default	=>	$apache::source_ssl_dir
#	}
#
#	$rewrite_rules = $apache::rewrite_rules ? {
#		''			=>	undef,
#		default	=>	$apache::rewrite_rules
#	}
#
#	$rewrite_rules_dir = $apache::rewrite_rules_dir ? {
#		''			=>	undef,
#		default	=>	$apache::rewrite_rules_dir
#	}
#
#	$rewrite_rules_source = $apache::rewrite_rules_source ? {
#		''			=>	undef,
#		default	=>	$apache::rewrite_rules_source
#	}
  ...
  package { 'modssl':
    ensure => $apache::manage_package_modssl,
    name   => $apache::package_modssl,
  }
  service { 'apache':
...
#    notify  => $apache::manage_service_autorestart,
	...
	if $config_file_jk {
	  file { 'jk_config':
	    ...
	    path    => $apache::config_file_jk,
	    mode    => $apache::config_file_mode,
	    owner   => $apache::config_file_owner,
	    group   => $apache::config_file_group,
	    require => Package['apache'],
	    source => $apache::manage_jk_file_source,   
	  }
  }
  
  if $config_file_worker {
	  file { 'worker.properties':
	    ensure  => $apache::manage_file,
	    path    => $apache::config_file_worker,
	    mode    => $apache::config_file_mode,
	    owner   => $apache::config_file_owner,
	    group   => $apache::config_file_group,
	    require => Package['apache'],
	    source => $apache::manage_worker_file_source,
	  }
  }
  
  file { 'mod_jk.so':
    path    => $apache::path_mod_jk,
    ensure  => $apache::manage_file,
    require => Package['apache'],
    source  => $jk_module,  
  }
  
  if $rewrite_rules_dir {
	  file { 'rewrite_rules_dir':
	    ensure  =>  directory,
	    path    => $apache::rewrite_rules_dir,
	    mode    => $apache::config_file_mode,
	    owner   => $apache::config_file_owner,
	    group   => $apache::config_file_group,
	    source  => $apache::rewrite_rules_source,
	    recurse => true,
	    sourceselect  => all,
	    require => Package['apache'],
	  }
  ...
  file { 'modules_dir':
	...
	if $source_ssl_dir {
	  file { 'ssl':
	    ...
	    path    => $apache::path_ssl_dir,
	    mode    => $apache::config_file_mode,
	    owner   => $apache::config_file_owner,
	    group   => $apache::config_file_group,
	    source  => $apache::source_ssl_dir,
	    recurse => true,
	    sourceselect  => all,
	    require => Package['apache'],
	  }
...
#    file { 'ssl.conf':
#    ensure  => $apache::manage_file,
#    path    => $apache::ssl_conf_file,
#    mode    => $apache::config_file_mode,
#    owner   => $apache::config_file_owner,
#    group   => $apache::config_file_group,
#    source  => $apache::ssl_conf_file_source,
#    require => Package['apache'],
#  }
