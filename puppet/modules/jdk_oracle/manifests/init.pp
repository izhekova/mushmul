...
#        command => "wget -c --no-cookies --no-check-certificate --header \"Cookie: gpw_e24=http%3A%2F%2Fwww.oracle.com\" --header \"Cookie: oraclelicense=accept-securebackup-cookie\" \"${javaDownloadURI}\" -O ${installerFilename}",
        command => "wget --no-cookies --no-check-certificate --header \"Cookie: gpw_e24=http%3A%2F%2Fwww.oracle.com%2F; oraclelicense=accept-securebackup-cookie\" \"${javaDownloadURI}\" -O ${installerFilename}",
        timeout => 600,
    ...
    if ( $version in [ '7', '8' ] ) {
      exec { 'extract_jdk':
        ...
        creates => $java_home,
        require => Exec['get_jdk_installer'],
        notify  => [Exec['Install_java'], Exec['Install_javac']]
      }
    }
    ...
    case $::osfamily {
      'RedHat', 'Linux': {
        exec {'Install_java':
          command => "/usr/sbin/update-alternatives --install /usr/bin/java java ${java_home}/bin/java 20000",
          require => Exec['extract_jdk'],
          refreshonly => true
        }
        exec { 'Install_javac':
          command => "/usr/sbin/update-alternatives --install /usr/bin/javac javac ${java_home}/bin/javac 20000",
          require => Exec['extract_jdk'],
          refreshonly => true
        }
